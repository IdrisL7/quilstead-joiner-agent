...
| Reject draft, recover through reset and prepare | pass |
| Approve and send; receipt stays in the same panel; no second approve path | pass |
...
| Offer another candidate after decline | not run: only one available candidate in the top three with this data |
...
| Keyboard order | nav, start date, recalculate, demo controls, candidate rows, actions; `:focus-visible` ring on all controls |
